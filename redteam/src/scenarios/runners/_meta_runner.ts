...
export async function runOne(name: string): Promise<ScenarioRun> {
    // `name` carries its category folder ("tenant/tenant-list-leak"), and this
    // runner is itself compiled into dist/scenarios/runners/, so one level up is
    // dist/scenarios/.
    const distFile = path.resolve(__dirname, "..", `${name}.js`);
