...
//! the TPM2-bound PoP roadmap ("Plan 1"), cancelled with this track in 2026-08.
//!
...
// comparison. See the TPM2-bound PoP roadmap, cancelled with this track.
// ─────────────────────────────────────────────────────────────────────────────
...
pub fn verify_aik_cert_chain(
    ...
    if trusted_roots.is_empty() {
        return Err(AttestationError::PartialImplementation(
            "no TPM2 vendor roots configured; place vendor DER certs at SAURON_TPM2_VENDOR_ROOTS_DIR (default /etc/sauronid/tpm2-roots/)"
        ));
    }
