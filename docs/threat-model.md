...
| **Gateway image substitution** | Attacker with registry write access, or a compromised release path, publishes a modified gateway image under a released tag. | Released images are signed keylessly at their digest (GitHub OIDC → Fulcio), and the release workflow verifies its own signature before completing. Customers verify the workflow identity with `cosign verify` and deploy the digest rather than the mutable tag (`core.image.digest` in the Helm values). The signed index digest also covers the attached SLSA provenance and SBOM. Procedure: `docs/verifying-what-you-run.md`. | Applies to customer-operated deployments. For an instance SauronID operates, image identity is NOT attested to the customer — see `docs/attestation-scope.md` |
