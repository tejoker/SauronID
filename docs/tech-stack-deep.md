...
│   │   ├── he/                     # Paillier homomorphic encryption
...
│   │   └── workflow-tracker.ts
│   └── test/                # E2E tests
├── clients/python/          # Python SDK + LLM adapters
│   ├── pyproject.toml
│   └── sauronid_client/
│       ├── client.py
...
Example (`schemas/fixtures/policy_banking_minimal.yaml`):
...
`core/src/tenancy/middleware.rs` is an axum `from_fn` middleware that:
...
`core/src/dp/` contains the DP module.
...
See [homomorphic-encryption.md](homomorphic-encryption.md).
...
See [tee-deployment.md](tee-deployment.md).
...
Circuits are written in Circom (target version 2.1.x) and live in [zkp/circuits/](../zkp/circuits/):
...
See [zk-action-logs.md](zk-action-logs.md).
...
Main entry: `agentic/src/index.ts`. Exports:
...
`agentic/src/enforcement.ts` exposes:
...
`agentic/src/ajwt.ts` uses `jose` for JWT signing and verification. A-JWTs are issued by the server and consumed by the SDK; the SDK does not sign A-JWTs.

### 15.5 Workflow tracker

`agentic/src/workflow-tracker.ts` is a stateful client-side recorder that hash-chains every action locally. The local chain is periodically reconciled with the server's anchored Merkle tree to detect tampering.
...
Distributed via PyPI as `sauronid-client`. Located at `clients/python/`.

### 16.2 Core client

`clients/python/sauronid_client/client.py`:
...
`clients/python/sauronid_client/enforcement.py` wraps `requests`-style HTTP clients to add signing + violation logging automatically.
...
All 16 pass in fail-closed mode. See [redteam-matrix.md](redteam-matrix.md) for current pass/fail status.
...
|---|---|---|
| `test.yml` | PR, push | `cargo test`, `npm test` in agentic/dashboard, pytest in clients/python |
...
1. Bump versions in `core/Cargo.toml`, `agentic/package.json`, `clients/python/pyproject.toml`, `dashboard/package.json`.
...
```bash
cd agentic && npm install && npm run build
...
See [tee-deployment.md](tee-deployment.md) for the full Nitro enclave deployment guide, including the enclave image build, attestation document collection, and PCR-based deployment verification.
...
- The Paillier HE implementation is constant-time. (It is not.)
