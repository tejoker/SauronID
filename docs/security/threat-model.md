...
|---|---|---|---|
| **Spoofing** | Caller forges admin auth | Bearer-key constant-time compare; min-32-byte production keys; multi-key list (`core/src/admin.rs::build_admin_auth_config` ~L52, ~L99-107) | Operator must protect the key; covered in [`key-rotation.md`](key-rotation.md) §1 |
...
| **Spoofing** | Malicious Solana RPC returns fake `getTransaction` | External verifier checks via a different RPC; signature recoverable from on-chain data | Operator configures multiple RPCs ([`disaster-recovery.md`](../operations/disaster-recovery.md) §2) |
...
| **Denial of service** | All calendars / RPCs simultaneously down | Bitcoin-only fallback (set `SAURON_SOLANA_ANCHOR_ENABLED=0`); Solana-only fallback acceptable for short term | DR runbooks: [`disaster-recovery.md`](../operations/disaster-recovery.md) §1-2 |
