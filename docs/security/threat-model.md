...
|---|---|---|---|
| **Spoofing** | Caller forges admin auth | Bearer-key constant-time compare; min-32-byte production keys; multi-key list (`core/src/admin.rs::build_admin_auth_config` ~L52, ~L99-107) | Operator must protect the key; covered in `docs/security/key-rotation.md` §1 |
...
| **Spoofing** | Malicious Solana RPC returns fake `getTransaction` | External verifier checks via a different RPC; signature recoverable from on-chain data | Operator configures multiple RPCs (`docs/operations/disaster-recovery.md` §2) |
...
| **Denial of service** | All calendars / RPCs simultaneously down | Bitcoin-only fallback (set `SAURON_SOLANA_ANCHOR_ENABLED=0`); Solana-only fallback acceptable for short term | DR runbooks: `docs/operations/disaster-recovery.md` §1-2 |
...
| **Gateway image substitution** | Attacker with registry write access, or a compromised release path, publishes a modified gateway image under a released tag. | Released images are signed keylessly at their digest (GitHub OIDC → Fulcio), and the release workflow verifies its own signature before completing. Customers verify the workflow identity with `cosign verify` and deploy the digest rather than the mutable tag (`core.image.digest` in the Helm values). The signed index digest also covers the attached SLSA provenance and SBOM. Procedure: `docs/security/verifying-what-you-run.md`. | Applies to customer-operated deployments. For an instance SauronID operates, image identity is NOT attested to the customer — see `docs/security/attestation-scope.md` |
| **Anchor provider downtime** | Bitcoin calendars + Solana RPCs all return errors simultaneously. | Backlog tolerated; queued anchors drain on recovery. Bitcoin-only or Solana-only mode acceptable for short outages. Runbooks: `docs/operations/disaster-recovery.md` §1-2. | n/a (infrastructure attack, not a runtime attack) |
