...
- Self-declared production boundary: [`threat-model.md`](../threat-model.md).
...
2. A statement signed with an Ed25519 key whose PEM is pinned under
   `release/reviewers/`, over the canonical compact JSON of
   `release/external-assessment.json` with `statement_signature_b64` omitted.
