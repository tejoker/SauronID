...
| --- | --- | --- |
| `core/src/attestation/tpm2.rs` | 1126 | TPM 2.0 quote parser, AIK signature verification, certificate-chain walker |
| `core/src/attestation/nitro.rs` + `nitro_pcr.rs` + `cbor.rs` | ~1700 | AWS Nitro document verification, hand-rolled CBOR / COSE_Sign1 parser, PCR comparison |
| `core/src/attestation/ed25519_self.rs` | 8 KB | Operator-rooted Ed25519 self-attestation |
| `core/src/attestation/abstraction.rs` | 39 | Vendor-neutral `AttestationVerifier` trait; backends are ZSTs dispatched statically |
| `core/src/attestation/handlers.rs` | 14 KB | `/v1/attestation/nitro/verify` |
...
There is also enclave scaffolding: `core/src/bin/nitro-enclave.rs` (278 lines)
...
document over vsock. `deploy/nitro/` has the EIF build and operator workflow;
`docs/tee-deployment.md` has the narrative.
...
hosting, and keep `deploy/nitro/` and the stub honest about their state in the
meantime.
